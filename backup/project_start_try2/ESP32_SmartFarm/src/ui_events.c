...
volatile unsigned long fan_last_request = 0;
volatile bool fan_request_pending = false;

//led toggle millis

volatile unsigned long led_last_request = 0;
volatile bool led_request_pending = false;

//smog toggle millis
volatile unsigned long smog_last_request = 0;
volatile bool smog_request_pending = false;

// 타이머 포인터 추가
static lv_timer_t *fan_status_timer = NULL;
static lv_timer_t *led_status_timer = NULL;
static lv_timer_t *smog_status_timer = NULL;
...
void request_fan_status(lv_timer_t * timer) {
    if (fan_request_pending && millis() - fan_last_request >= 100) {
        ...
        while (!Wire.available() && millis() - start_time < 50);

        if (Wire.available()) {
            char fan_state = Wire.read();
            if (lv_obj_is_valid(ui_fan)) {
...
void fan_toggle(lv_event_t * e) {
    ...
    if (fan_status_timer != NULL) {
        lv_timer_del(fan_status_timer);
    }
    fan_status_timer = lv_timer_create(request_fan_status, 300, NULL);
    lv_timer_set_repeat_count(fan_status_timer, 1);
}
...
void toggle_led(lv_event_t * e) {
    ...
    if (led_status_timer != NULL) {
        lv_timer_del(led_status_timer);
    }
    led_status_timer = lv_timer_create(request_led_status, 300, NULL);
    lv_timer_set_repeat_count(led_status_timer, 1);
}
...
void measure_heart_rate(lv_timer_t * timer) {
    ...
    if (isnan(heartRate) || heartRate <= 0) {  // 🔹 NaN 값 체크
        Serial.println("❌ 심박수 값이 비정상적입니다.");
        heartRate = -1;  
    } else {
        Serial.print("심박수: ");
...
#define AUDIO_FILE_PATH "/touch.wav"
...
void on_button_pressed(lv_event_t * e) {
    Serial.println("🖲 버튼 터치됨, 효과음 실행!");
        if (!SD.begin(SD_CS)) {  // SD 카드 상태 확인
        Serial.println("❌ SD 카드가 준비되지 않음. 효과음 실행 불가!");
        return;
    }
...
void update_temp_hum(lv_timer_t * timer) {
    // UI 객체 유효성 검사 추가
    if (!lv_obj_is_valid(ui_tempData) || !lv_obj_is_valid(ui_humData) ||
        !lv_obj_is_valid(ui_tempBar) || !lv_obj_is_valid(ui_humBar)) {
        return;
    }
