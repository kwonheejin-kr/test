...
void request_smog_status(lv_timer_t * timer) {
    if (smog_request_pending && millis() - smog_last_request >= 100) {
        Wire.requestFrom(SLAVE_ADDR, 1);  
        unsigned long start_wait = millis();
        while (!Wire.available() && millis() - start_wait < 50);  // 최대 50ms 대기
        ...
        if (Wire.available()) {
            ...
            Serial.println("가습기 상태 요청 실패: 응답 없음.");
        }

        smog_request_pending = false;
    }
}
...
void save_measurement_data(int32_t hr, int32_t spo2) {
    ...
    if (!check_sd_card_status()) {
        lv_label_set_text(ui_HeartResult, "SD 카드 오류\n데이터 저장 실패");
        return;
    }
    ...
    // 날짜별 파일 생성 (YYYY-MM-DD.csv)
    snprintf(filename, sizeof(filename), "/measurements/%04d-%02d-%02d.csv", 
             timeinfo->tm_year + 1900, timeinfo->tm_mon + 1, timeinfo->tm_mday);
    
    // 파일이 존재하는지 확인하고 헤더 추가
    ...
    if (dataFile) {
        // 새 파일이면 헤더 추가
        if (!file_exists) {
            dataFile.println("Timestamp,HeartRate,SpO2");
        }
        ...
        char data_line[64];
        snprintf(data_line, sizeof(data_line), "%s,%ld,%ld", timestamp, hr, spo2);
        ...
        Serial.println("파일 열기 실패");
        lv_label_set_text(ui_HeartResult, "파일 저장 오류\n다시 시도해주세요");
    }
}
...
void a2dp_sink_init() {
    ...
    Serial.println("블루투스 연결 완료");
}

void play_touch_sound() {
    File audioFile = SD.open(TOUCH_SOUND_PATH);  // 효과음 파일 열기
    if (!audioFile) {
...
void ui_event_ControlRightUp(lv_event_t * e) {
    ...
    if(event_code == LV_EVENT_CLICKED) {
        ...
        // 상태 확인 타이머 시작 (없으면 생성)
        static lv_timer_t * smog_timer = NULL;
        if (smog_timer == NULL) {
            smog_timer = lv_timer_create(request_smog_status, 100, NULL);
        }
    }
}
...
void measure_heart_rate(lv_timer_t * timer) {
    ...
    // uce21uc815 uc9c4ud589ub960 uacc4uc0b0
    unsigned long elapsed = millis() - start_time;
    int progress = (elapsed * 100) / 5000;  // 5ucd08 uae30uc900
    
    if (progress > 100) progress = 100;
    
    // uc9c4ud589ub960 ud45cuc2dc
    lv_label_set_text_fmt(ui_Measuring, "Measuring... %d%%", progress);
    
    // uc13cuc11c ub370uc774ud130 uc77duce58
    if (particleSensor.available()) {
        ...
        // 100uac1c uc0d8ud50c uc218uc9d1 uc644ub8cc
        if (buffer_index >= 100) {
            // uc2ecubc15uc218/SpO2 uacc4uc0b0
            maxim_heart_rate_and_oxygen_saturation(
                ir_buffer, 100, red_buffer, &spo2, &validSPO2, &heartRate, &validHeartRate);
            
            buffer_index = 0;
            
            // uc720ud6a8ud55c uacb0uacfc uac12uc774 uc788uc744 uacbduc6b0 ud45cuc2dc
            if (validHeartRate && validSPO2) {
                ...
                // uce21uc815 uc644ub8cc
                if (elapsed >= 5000) {
                    lv_label_set_text_fmt(ui_HeartResult, "uc2ecubc15uc218: %ld bpm\nSpO2: %ld%%", heartRate, spo2);
                    lv_label_set_text(ui_Measuring, "uce21uc815 uc644ub8cc");
                    
                    // ub370uc774ud130 uc800uc7a5
                    save_measurement_data(heartRate, spo2);
                    
                    // ud0c0uc774uba38 uc81cuac70
                    lv_timer_del(timer);
                    is_measuring = false;
                }
            }
...
void HeartBeatMeasuringStart(lv_event_t * e)
{
    Serial.println("HeartMeasuring ud654uba74 ub85cub4dcub428, 'Touch Plz' ud45cuc2dc.");
    lv_label_set_text(ui_Measuring, "Touch Plz");

    start_time = millis();
    is_measuring = false;
    
    // uc2ecubc15 uc13cuc11c ucd08uae30ud654
    if (!particleSensor.begin()) {
        Serial.println("MAX30105 uc13cuc11c ucd08uae30ud654 uc2e4ud328");
        lv_label_set_text(ui_Measuring, "uc13cuc11c uc624ub958");
        return;
    }
    
    // uc13cuc11c uc124uc815
