...
bool manualHumidifierControl = false; // 수동 모드에서 가습기 제어 여부
...
void setup() {
  ...
  // LED 초기화 - 백라이트 제어용
  pinMode(5, OUTPUT);
  digitalWrite(5, HIGH);  // LED 켜기 (백라이트 ON)
...
void sendCommand(char cmd, String param = "") {
  // 가습기 명령인 경우 추가 검증 수행
  if (cmd == CMD_HUM_ON) {
    // 수동 모드에서 의도치 않은 가습기 켜기 명령 방지
    if (!autoModeEnabled && !manualHumidifierControl) {
      Serial.println("[BLOCKED] 수동 모드에서 의도치 않은 가습기 켜기 명령 차단");
      return;
    }
    Serial.print("★★★ 가습기 켜기 명령 전송: 자동모드=");
  ...
  if (separatorIndex > 0) {
    ...
    if (key.startsWith("BTN")) {
      if (key == "BTN_FAN") {
        ...
        manualHumidifierControl = true; // 수동 모드에서 가습기 제어
...
void drawMainScreen() {
  ...
  // 현재 모드 표시
  tft.fillRect(tft.width() - 60, 5, 55, 20, autoModeEnabled ? AUTO_COLOR : MANUAL_COLOR);
  tft.setTextSize(1);
  tft.setTextColor(TFT_BLACK);
  tft.drawString(autoModeEnabled ? "AUTO" : "MANUAL", tft.width() - 43, 10);
  tft.drawRect(tft.width() - 61, 4, 57, 22, TEXT_COLOR);
...
void updateDeviceStatus() {
  ...
  tft.setTextColor(TFT_BLACK); // 글자색 검정으로 변경
  tft.drawString(autoModeEnabled ? "AUTO" : "MANUAL", tft.width() - 43, 10);
  tft.drawRect(tft.width() - 61, 4, 57, 22, TEXT_COLOR); // 테두리 추가
