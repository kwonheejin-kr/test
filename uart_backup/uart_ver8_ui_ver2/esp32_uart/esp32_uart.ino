...
const int TEMP_AREA_Y = 65;    // 온도 영역
const int HR_AREA_Y = 170;     // 심박수 영역
...
void updateDeviceStatus() {
  ...
  tft.setTextColor(TEXT_COLOR);
  tft.drawString("Fan", 40, statusY + 5);
  ...
  tft.setTextColor(TEXT_COLOR);
  tft.drawString("LED", 140, statusY + 5);
  ...
  tft.setTextColor(TEXT_COLOR);
  tft.drawString("Humid", 240, statusY + 5);
...
void updateSensorDisplay() {
  ...
  if (heartRateValue > 0) {
    ...
    // 심박수 애니메이션 (깜빡임 효과)
    static bool heartBlink = false;
    static unsigned long lastHeartBlink = 0;
    
    if (millis() - lastHeartBlink > 60000 / heartRateValue / 2) { // 심박수에 맞춰 깜빡임
      heartBlink = !heartBlink;
      lastHeartBlink = millis();
      if (heartBlink) {
        tft.fillCircle(18, HR_AREA_Y + 15, 6, HR_COLOR); // 심장 깜빡임
      } else {
        tft.fillCircle(18, HR_AREA_Y + 15, 6, 0x3186); // 어두운 빨간색
      }
    }
  } else {
